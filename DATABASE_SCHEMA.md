...
* **`planner_misc_state`** — компактное хранилище базовых настроек (режим, фильтры, флаги UI). Используется для объединения с данными CRM и таблицами маршрутов при сборке снапшота для клиентов. Ключ `base` хранит нормализованные настройки, а ключ `full` — последнюю полную версию снапшота в качестве резервного источника для инициализации, если вспомогательные таблицы ещё пусты.【F:migrations/018_remove_snapshot_storage.sql†L107-L113】【F:server.js†L773-L810】
